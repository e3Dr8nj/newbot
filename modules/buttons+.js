//
//_____________----ButtonInteraction
exports.onButtonClick=async(client,interaction)=>{try{
         
           await interaction.reply({content:`${interaction.user.tag} ${interaction.customId}`,ephemeral: true})
...
module.exports.commands.command2={disable:false,aliase:'testb', run:async(client,message,args)=>{try{
 ...
 // let role = message.guild.roles(r=>r.name=='Cупермодератор')
  //message.channnel.send(role.toString)
